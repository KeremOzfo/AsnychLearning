...
from Pytorch import nn_classes
from Pytorch import data_loader
from Pytorch import ps_functions
from Pytorch import SGD_custom
...
prev_Optimizer = SGD_custom.define_optimizer(prev_model,lr,momentum, weight_decay)
...
ps_functions.synch_weight(avg_model, prev_model)
ps_functions.synch_weight(prev_model, bcast_model)

runs = 30000
for x in range(4):
    w_index = 0
    results = np.empty([1, 301])
    res_ind = 0
    for r in tqdm(range(runs)):
        # index of the worker doing local SGD
        w_index = w_index % N_w
        for data in trainloaders[w_index]:
            inputs, labels = data
            inputs, labels = inputs.to(device), labels.to(device)
            optimizers[w_index].zero_grad()
            preds = nets[w_index](inputs)
            loss = criterions[w_index](preds, labels)
            loss.backward()
            optimizers[w_index].step()
            break
        #corrected momentum
        ps_functions.model_Bcast(bcast_Optimizer,prev_Optimizer,optimizers[w_index],r,alpha)
        # assaign bcast_model to prev model
        ps_functions.momentum_change(bcast_Optimizer, prev_Optimizer)
        ...
        if r == 15000:
            for n in range(N_w):
                if x==0:
                    ps_functions.lr_change(1e-2,optimizers[n])
                elif x==1:
                    ps_functions.lr_change(0.015, optimizers[n])
                elif x==2:
                    ps_functions.lr_change(0.02, optimizers[n])
                else:
                    ps_functions.lr_change(0.025, optimizers[n])
        if r == 22500:
            for n in range(N_w):
                if x == 0:
                    ps_functions.lr_change(1e-3, optimizers[n])
                elif x == 1:
                    ps_functions.lr_change(0.0015, optimizers[n])
                elif x == 2:
                    ps_functions.lr_change(0.002, optimizers[n])
                else:
                    ps_functions.lr_change(0.0025, optimizers[n])

        # moving to next worker
        w_index += 1
    f = open('asynchResults' + '.txt', 'ab')
